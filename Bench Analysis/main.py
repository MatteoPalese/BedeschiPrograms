...
data = \
{
    ...
    "Angle between buckets": [[None, None], ['2 * pi  / data["Number of buckets"][0]', '360 / data["Number of buckets"][0]']],
    "Time in which bucket wheel rotates of a": [None, '60 / data["Bucket wheel rotational speed"][0] / data["Number of buckets"][0]'],
    "Buckets discharge": [[None, None], ['data["Bucket wheel rotational speed"][0] * data["Number of buckets"][0]', 'data["Buckets discharge"][0][0] / 60']],
    "Pitch between buckets": [[None, None, None], ['3.14 * data["Diameter (on cutting edge)"][0] / data["Number of buckets"][0]', 'data["Maximum selected boom slewing speed"][0] / 60', 'data["Max Horizontal distance cutting edge/boom slew. CL"][0][1] * data["Max Horizontal distance cutting edge/boom slew. CL"][0][0]']],
    ...
    "Reclaiming Angle at which max slewing speed is reached": [[None, None], ['data["Reclaiming Angle at which max slewing speed is reached"][0][1] * 180 / pi', 'acos(data["Max Horizontal distance cutting edge/boom slew. CL"][0][1] / data["Max Horizontal distance cutting edge/boom slew. CL"][0][3])']],
    "Bench height": [None, 'data["Stockpile height"][0] / data["Number of selected benches"][0]'],
    "Number of buckets in the material": [None, 'data["Scarp angle"][0] / data["Angle between buckets"][0][0]'],
    "Numb. of buckets in the material considered for calc.": [None, 'ceil(data["Number of buckets in the material"][0])'],
    "Angle of 1° Bucket": [None, '0'],
    "Angle of 2° Bucket": [None, 'data["Angle between buckets"][0] if data["Numb. of buckets in the material considered for calc."][0] >=2 else "n.a."'],
    "Angle of 3° Bucket": [None, 'data["Angle between buckets"][0] * 2 if data["Numb. of buckets in the material considered for calc."][0] >=3 else "n.a."'],
    "Angle of 4° Bucket": [None, 'data["Angle between buckets"][0] * 3 if data["Numb. of buckets in the material considered for calc."][0] >=4 else "n.a."'],
    # Check Data
    "Scarp angle": [[None, None, None], ['acos(data["Diameter (on cutting edge)"][0] / 2 - data["Bench height"][0]) / (data["Diameter (on cutting edge)"][0] / 2) if data["Diameter (on cutting edge)"][0] >= data["Bench height"][0] else pi / 2 + asin((data["Bench height"][0] - data["Diameter (on cutting edge)"][0] / 2) / (data["Diameter (on cutting edge)"][0] / 2)) ', 'data["Scarp angle"][0][0] * 180 / pi', '"ok" if data["Scarp angle"][0][1] <110 and if data["Scarp angle"][0][1] < 720 / data["Number of buckets"][0] else "verify"']],
    "Froud factor": [[None, None], ['data["Tangential Speed"][0][0] / (data["Diameter (on cutting edge)"][0] / 2 * 9.81) ** (1 / 2)', '"ok" if 0.2<data["Froud factor"][0][0]<0.7 else "verify"']],
    "Bucket Wheel Tangential Limit Speed": [None, '(9.81 * data["Diameter (on cutting edge)"][0] / 2) ** 0.5'],
